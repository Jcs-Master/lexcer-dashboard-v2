...
export default function AciPaths() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await aciAPI.generatePaths(fd)
      setResult(res.data)
    } catch (err) {
        ...
        <div className="card">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div onDragOver={(e) => e.preventDefault()} onDrop={handleDrop}
              ...
              <input type="file" accept=".xls,.xlsx" onChange={handleFile} className="hidden" id="aci-file" />
              <label htmlFor="aci-file" className="cursor-pointer">
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="card p-4">
              ...
              <p className="text-sm text-slate-200 mt-1 font-mono">{result.summary.tenants.join(', ') || '-'}</p>
            </div>
            <div className="card p-4">
              <p className="text-xs text-slate-500">Applications</p>
              <p className="text-sm text-slate-200 mt-1 font-mono">{result.summary.applications.join(', ') || '-'}</p>
            </div>
            <div className="card p-4">
              <p className="text-xs text-slate-500">EPGs</p>
              <p className="text-sm text-slate-200 mt-1 font-mono">{result.summary.epgs.join(', ') || '-'}</p>
            </div>
          </div>
