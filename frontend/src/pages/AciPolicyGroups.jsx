...
export default function AciPolicyGroups() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await aciAPI.generatePolicyGroups(fd)
      setResult(res.data)
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Genera XML de creacion y borrado para Interface Policy Groups ACI desde Excel</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="card">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div onDragOver={(e) => e.preventDefault()} onDrop={handleDrop}
              ...
              <input type="file" accept=".xls,.xlsx" onChange={handleFile} className="hidden" id="ipg-file" />
              <label htmlFor="ipg-file" className="cursor-pointer">
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {['NAME', 'TYPE', 'DESCRIPTION', 'SPEED_POLICY', 'CDP_POLICY', 'LLDP_POLICY', 'STP_POLICY', 'AAEP', 'LACP_POLICY'].map(c => (
              <span key={c} className="px-2 py-1 rounded-md bg-slate-800 text-xs font-mono text-cyan-400 border border-slate-700">
                {c}
              </span>
            ))}
          </div>
          <p className="text-xs text-slate-500">TYPE: LINK | PC | VPC. LACP_POLICY solo para PC/VPC.</p>
        </div>
      </div>
      ...
      {result && (
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-5">
            ...
            <div className="card p-4 text-center">
              <p className="text-xs text-slate-500 uppercase tracking-wider">Link</p>
              <p className="text-2xl font-mono font-bold text-blue-400 mt-1">{result.summary.link}</p>
            </div>
            <div className="card p-4 text-center">
              <p className="text-xs text-slate-500 uppercase tracking-wider">PC</p>
              <p className="text-2xl font-mono font-bold text-purple-400 mt-1">{result.summary.pc}</p>
            </div>
            <div className="card p-4 text-center">
              <p className="text-xs text-slate-500 uppercase tracking-wider">VPC</p>
              <p className="text-2xl font-mono font-bold text-orange-400 mt-1">{result.summary.vpc}</p>
            </div>
          </div>
